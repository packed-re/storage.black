<?php
	...
	switch($_SERVER["REQUEST_METHOD"])
	{
		case "GET":
			...
			exit(pack("Qa*", $read_chunk_size, json_encode($db->ListFiles($session->account_id, $data_id)))); // chunk size + list
		...
		case "POST":
			...
			switch ($file_header->action)
			{
				...
				case 2: // delete
					if($file_exists && unlink($file_name) === false) // maybe the file doesnt exist, while for whatever reason the listing in the db does
